    ...
    <div class="bg-white rounded-xl shadow-2xl w-full max-w-sm p-6 text-sm">
      ...
      <form @submit.prevent="submitForm" class="space-y-4">
        ...
        <div>
          ...
          <input v-model="form.subject" type="text" required class="border border-gray-300 rounded-lg w-full px-3 py-2" />
        </div>
        <div>
          <label class="block mb-1 text-sm font-medium text-gray-700">Message</label>
          <input v-model="form.message" type="text" required class="border border-gray-300 rounded-lg w-full px-3 py-2" />
        </div>
...
message: this.data?.message || ''
  ...
  methods: {
    async submitForm() {
      try {
        if ("Edit" === "Add") {
          await this.$apiPost("/contacts", this.form);
        } else {
          await this.$apiPatch(`/contacts/${this.data.id}/`, this.form);
        }
        this.$emit("saved");
        this.$emit("close");
