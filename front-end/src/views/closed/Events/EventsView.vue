        ...
        <table class="min-w-full text-sm divide-y divide-gray-200">
          <thead class="bg-gray-100 text-gray-700 uppercase text-xs font-semibold">
            <tr>
              ...
              <th class="px-6 py-3 text-left">Title</th><th class="px-6 py-3 text-left">Description</th><th class="px-6 py-3 text-left">Date</th><th class="px-6 py-3 text-left">Location</th><th class="px-6 py-3 text-left">Category</th><th class="px-6 py-3 text-left">Image</th>
          ...
          <tbody class="bg-white divide-y divide-gray-200">
            ...
            >
              ...
              <td class="px-6 py-4 whitespace-nowrap">{{ item.title }}</td><td class="px-6 py-4 whitespace-nowrap">{{ item.description }}</td><td class="px-6 py-4 whitespace-nowrap">{{ item.date }}</td><td class="px-6 py-4 whitespace-nowrap">{{ item.location }}</td><td class="px-6 py-4 whitespace-nowrap">{{ item.category }}</td><td class="px-6 py-4 whitespace-nowrap">{{ item.image }}</td>
    ...
    <div class="md:hidden space-y-4">
      ...
      >
        <div class="flex justify-between mb-3">
          <h2 class="font-bold text-gray-800">Events #{{ index + 1 }}</h2>
...
export default {
  components: { AddEvents, EditEvents },
  data() {
    return { items: [], showModal: false, editMode: false, selectedItem: null };
  },
  methods: {
    async fetchItems() {
      this.items = (await this.$apiGet("/events")).data || [];
    },
    ...
    async deleteItem(id) {
      if (confirm("Are you sure?")) {
        await this.$apiDelete(`/events/${id}/`);
        this.fetchItems();
      }
    }
  },
