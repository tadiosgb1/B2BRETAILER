    ...
    <div class="bg-white rounded-xl shadow-2xl w-full max-w-sm p-6 text-sm">
      ...
      <form @submit.prevent="submitForm" class="space-y-4">
        ...
        <div>
          ...
          <input v-model="form.location" type="text" required class="border border-gray-300 rounded-lg w-full px-3 py-2" />
        </div>
        <div>
...
image: this.data?.image || ''
  ...
  methods: {
    async submitForm() {
      try {
        if ("Edit" === "Add") {
          await this.$apiPost("/events", this.form);
        } else {
          await this.$apiPatch(`/events/${this.data.id}/`, this.form);
        }
        this.$emit("saved");
        this.$emit("close");
