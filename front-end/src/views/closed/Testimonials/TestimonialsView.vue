        ...
        <table class="min-w-full text-sm divide-y divide-gray-200">
          <thead class="bg-gray-100 text-gray-700 uppercase text-xs font-semibold">
            <tr>
              ...
              <th class="px-6 py-3 text-left">Name</th><th class="px-6 py-3 text-left">Designation</th><th class="px-6 py-3 text-left">Message</th><th class="px-6 py-3 text-left">Photo</th>
          ...
          <tbody class="bg-white divide-y divide-gray-200">
            ...
            >
              ...
              <td class="px-6 py-4 whitespace-nowrap">{{ item.name }}</td><td class="px-6 py-4 whitespace-nowrap">{{ item.designation }}</td><td class="px-6 py-4 whitespace-nowrap">{{ item.message }}</td><td class="px-6 py-4 whitespace-nowrap">{{ item.photo }}</td>
    ...
    <div class="md:hidden space-y-4">
      ...
      >
        <div class="flex justify-between mb-3">
          <h2 class="font-bold text-gray-800">Testimonials #{{ index + 1 }}</h2>
...
<script>
import AddTestimonials from "./AddTestimonials.vue";
import EditTestimonials from "./EditTestimonials.vue";

export default {
  components: { AddTestimonials, EditTestimonials },
  data() {
    return { items: [], showModal: false, editMode: false, selectedItem: null };
  },
  methods: {
    async fetchItems() {
      this.items = (await this.$apiGet("/testimonials")).data || [];
    },
    ...
    async deleteItem(id) {
      if (confirm("Are you sure?")) {
        await this.$apiDelete(`/testimonials/${id}/`);
        this.fetchItems();
      }
    }
  },
