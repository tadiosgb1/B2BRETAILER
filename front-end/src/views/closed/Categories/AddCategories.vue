    ...
    <div class="bg-white rounded-xl shadow-2xl w-full max-w-sm p-6 text-sm">
      ...
      <form @submit.prevent="submitForm" class="space-y-4">
        ...
        <div>
          ...
          <input v-model="form.slug" type="text" required class="border border-gray-300 rounded-lg w-full px-3 py-2" />
        </div>
...
slug: this.data?.slug || ''
  ...
  methods: {
    async submitForm() {
      try {
        if ("Add" === "Add") {
          await this.$apiPost("/categories", this.form);
        } else {
          await this.$apiPatch(`/categories/${this.data.id}/`, this.form);
        }
        this.$emit("saved");
        this.$emit("close");
