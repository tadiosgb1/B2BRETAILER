        ...
        <table class="min-w-full text-sm divide-y divide-gray-200">
          <thead class="bg-gray-100 text-gray-700 uppercase text-xs font-semibold">
            <tr>
              ...
              <th class="px-6 py-3 text-left">Name</th><th class="px-6 py-3 text-left">Logo</th><th class="px-6 py-3 text-left">Website</th><th class="px-6 py-3 text-left">Description</th>
          ...
          <tbody class="bg-white divide-y divide-gray-200">
            ...
            >
              ...
              <td class="px-6 py-4 whitespace-nowrap">{{ item.name }}</td><td class="px-6 py-4 whitespace-nowrap">{{ item.logo }}</td><td class="px-6 py-4 whitespace-nowrap">{{ item.website }}</td><td class="px-6 py-4 whitespace-nowrap">{{ item.description }}</td>
    ...
    <div class="md:hidden space-y-4">
      ...
      >
        <div class="flex justify-between mb-3">
          <h2 class="font-bold text-gray-800">Partners #{{ index + 1 }}</h2>
...
<script>
import AddPartners from "./AddPartners.vue";
import EditPartners from "./EditPartners.vue";

export default {
  components: { AddPartners, EditPartners },
  data() {
    return { items: [], showModal: false, editMode: false, selectedItem: null };
  },
  methods: {
    async fetchItems() {
      this.items = (await this.$apiGet("/partners")).data || [];
    },
    ...
    async deleteItem(id) {
      if (confirm("Are you sure?")) {
        await this.$apiDelete(`/partners/${id}/`);
        this.fetchItems();
      }
    }
  },
