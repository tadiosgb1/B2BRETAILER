...
<template>
  <div class="p-6 bg-gray-50 min-h-screen text-sm text-gray-800">
    ...
    <div class="flex items-center justify-between mb-6 border-b pb-4 border-gray-200">
      <h1 class="text-lg font-bold text-gray-800">Awards</h1>
        ...
        <table class="min-w-full text-sm divide-y divide-gray-200">
          <thead class="bg-gray-100 text-gray-700 uppercase text-xs font-semibold">
            <tr>
              ...
              <th class="px-6 py-3 text-left">Title</th><th class="px-6 py-3 text-left">Description</th><th class="px-6 py-3 text-left">Year</th><th class="px-6 py-3 text-left">Image</th>
          ...
          <tbody class="bg-white divide-y divide-gray-200">
            ...
            >
              ...
              <td class="px-6 py-4 whitespace-nowrap">{{ item.title }}</td><td class="px-6 py-4 whitespace-nowrap">{{ item.description }}</td><td class="px-6 py-4 whitespace-nowrap">{{ item.year }}</td><td class="px-6 py-4 whitespace-nowrap">{{ item.image }}</td>
    ...
    <div class="md:hidden space-y-4">
      ...
      >
        <div class="flex justify-between mb-3">
          <h2 class="font-bold text-gray-800">Awards #{{ index + 1 }}</h2>
...
<script>
import AddAwards from "./AddAwards.vue";
import EditAwards from "./EditAwards.vue";

export default {
  components: { AddAwards, EditAwards },
  data() {
    return { items: [], showModal: false, editMode: false, selectedItem: null };
  },
  methods: {
    async fetchItems() {
      this.items = (await this.$apiGet("/awards")).data || [];
    },
    ...
    async deleteItem(id) {
      if (confirm("Are you sure?")) {
        await this.$apiDelete(`/awards/${id}/`);
        this.fetchItems();
      }
    }
  },
