<template>
  <div class="h-screen flex flex-col overflow-hidden">  <Header  class="fixed top-0 w-full z-50 shadow-md bg-white " />
     <main class="flex-1 overflow-y-auto  ">
      <div class="container mx-auto py-8 px-4 sm:px-6 lg:px-8">
      <!-- Loading / Not Found -->
      <div v-if="loading" class="fixed inset-0 bg-black bg-opacity-30 flex justify-center items-center z-50">
        ...
        <div class="grid grid-cols-1 md:grid-cols-12 gap-6 lg:gap-8">
          ...
          <div class="md:col-span-5 relative">
            <div class="w-full h-96 rounded-xl overflow-hidden mb-4 cursor-pointer border border-gray-200 relative"
              ...
              <!-- Image fraction -->
              <div
                class="absolute top-2 right-2 bg-black bg-opacity-50 text-white px-2 py-1 rounded text-sm font-semibold">
                {{ selectedIndex + 1 }}/{{ totalImages }}
              </div>

              <!-- Navigation -->
              <button @click.stop="prevImage" :disabled="selectedIndex === 0"
            ...
            <div v-if="product.images?.length" class="flex gap-2 overflow-x-auto py-2">
              <div v-for="(img, index) in product.images" :key="index"
                ...
                :class="{ 'ring-2 ring-orange-500': selectedIndex === index }" @click="selectedIndex = index">
                <img :src="this.$proxiedImage(img.original_url)" class="w-full h-full object-cover" />
              </div>
            </div>
            <div class="p-4 bg-white border border-gray-200 rounded-md mt-10">
              <!-- Header -->
              <div class="flex items-center justify-between mb-3">
                ...
                <div>
                  <button v-if="token" @click="openReviewModal"
                    class="px-3 py-1 bg-orange-500 text-white text-xs font-semibold rounded-lg hover:bg-orange-600 transition">
                    Review
                  </button>
                  <p v-else class="flex items-center gap-1 text-orange-500 text-xs">
                    <span>🔒</span>
              ...
              <div v-if="product.total_reviews > 0" class="flex space-x-3 overflow-x-auto pb-2">
                <div v-for="r in product.reviews" :key="r.id"
                  class="flex-shrink-0 w-64 bg-gray-50 border border-gray-100 rounded-lg p-3 shadow-sm hover:shadow-md transition">
                  <div class="flex items-center justify-between mb-1">
                    <span class="text-sm font-semibold text-gray-700">{{ r.user_name || 'Anon' }}</span>
                    <span class="text-xs text-gray-400">{{ r.date | formatDate }}</span>
                  </div>
                  <!-- Star rating -->
                  <div class="flex text-yellow-400 text-xs mb-1">
                    <i v-for="i in 5" :key="i" class="fas fa-star"
                      :class="i <= r.rating ? 'text-yellow-400' : 'text-gray-300'"></i>
                  </div>
                  <p class="text-gray-600 text-sm line-clamp-3">{{ r.review }}</p>
                </div>
              </div>

              <!-- No reviews -->
              <p v-else class="text-gray-500 italic text-sm">Be the first to review this product.</p>
            </div>
          ...
          <div class="md:col-span-7 space-y-4">
            ...
            <div class="pb-2 border-b border-gray-100">
              <h1 class="text-2xl font-extrabold text-gray-600 leading-snug tracking-tight truncate ">
              ...
              <div class="mt-3 flex items-center justify-between">
                ...
                <!-- Stars + numeric rating + descriptive text on the right -->
                <div class="flex items-center gap-3">
                  <!-- Stars -->
                  <div class="flex items-center space-x-1">
                    <template v-for="i in 5" :key="i">
                      <!-- Full star -->
                      <svg v-if="i <= Math.floor(product.rate)" class="w-5 h-5 text-orange-500" fill="currentColor"
                        viewBox="0 0 20 20">
                        <path
            ...
            <!-- Product Meta -->
            <div
              class="pt-2 bg-gray-50 p-4 rounded-lg grid grid-cols-2 gap-x-6 gap-y-3 text-sm font-medium border border-gray-200">
              ...
              <div v-if="product.skus?.length">
                <div v-for="sku in product.skus.slice(0, 1)" :key="sku.id"
                  ...
                  <span class="font-semibold flex items-center">
                    ...
                    <span
                      :class="{ 'text-red-600': sku.stockCount < 10, 'text-green-600': sku.stockCount >= 10, 'text-gray-500': !sku.stockCount }">
                      {{ sku.stockCount ?? 0 }}
                    </span>
                  </span>
                </div>
            ...
            <div v-if="product.discount"
              class="mt-4 p-3 bg-orange-100 border-l-4 border-orange-400 rounded-r-lg flex items-center gap-3">
            ...
            <div
              class="mt-6 p-4 bg-orange-50 rounded-lg border border-orange-200 flex flex-col sm:flex-row items-center justify-between gap-4">
              <div class="flex items-center gap-4">
                <label for="quantity-input" class="text-lg font-medium text-gray-700 flex-shrink-0">Quantity:</label>
                <input id="quantity-input" type="number" v-model.number="quantity" min="1"
  ...
  class="w-full sm:w-auto px-6 py-3 border-2 border-orange-600 text-orange-600 font-bold rounded-lg 
         hover:bg-orange-600 hover:text-white transition flex items-center justify-center gap-2">
  <i class="fas fa-warehouse text-lg"></i> Add to cart
</button>
            ...
            <div v-if="product.attributes?.length">
              ...
              <ul
                class="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-3 gap-x-8 gap-y-3 text-gray-700 p-4 border rounded-lg bg-gray-50">
                <li v-for="attr in product.attributes" :key="attr.id"
                  class="flex justify-between items-start border-b pb-1 border-dashed">
  ...
  <div class="grid grid-cols-2 sm:grid-cols-3 md:grid-cols-4 lg:grid-cols-6 gap-4">
    <div v-for="p in relatedProducts.slice(0, 6)" :key="p.id"
      class="border rounded-lg p-4 hover:shadow-lg transition cursor-pointer bg-white flex flex-col"
      @click="goToProductDetail(p)">

      <!-- Product Image -->
      <img :src="this.$proxiedImage(p.imageSrc)"
        class="w-full h-32 sm:h-36 md:h-40 object-contain rounded mb-3 border border-gray-100" 
        alt="Related Product" />

      <!-- Product Name (truncate professionally) -->
      <h3 class="font-semibold text-sm text-gray-800 line-clamp-2 mb-1 truncate">
        {{ p.name }}
      </h3>
      ...
      <!-- Modals -->
      <div v-if="modalOpen"
        class="fixed inset-0 bg-black bg-opacity-70 flex items-center justify-center z-50 overflow-auto">
        <div class="relative bg-white rounded-lg p-2 max-w-4xl w-full mx-4 sm:mx-8">
          ...
          <img :src="this.$proxiedImage(modalImage)" class="w-full h-auto rounded max-h-[90vh]"
            alt="Enlarged Product Image" />
        </div>
      </div>

      <!-- Review Modal -->
      <div v-if="ReviewModalOpen"
        class="fixed inset-0 bg-black bg-opacity-70 flex items-center justify-center z-50 overflow-auto">
        <div class="relative bg-white rounded-lg p-6 max-w-md w-full mx-4 sm:mx-8 shadow-2xl">
          ...
          <div class="space-y-4">
            ...
            <button @click="submitReview()" :disabled="reviewRating === 0 || reviewText.length < 5"
      ...
      <ChooseWarehouseAddToCart :open="WarehouseModalOpen" :product="product" :quantity="quantity"
        @close="WarehouseModalOpen = false" @added="onAddedToCart" />
...
export default {
  ...
  computed: {
    formattedPrice() {
      const price = this.product.skus[0]?.sell_price ?? 0;
      return price.toString().padStart(3, '0');
    },
    selectedImage() {
